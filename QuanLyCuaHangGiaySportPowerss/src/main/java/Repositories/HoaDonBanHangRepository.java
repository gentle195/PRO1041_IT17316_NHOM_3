    ...
    public void update(HoaDon hoaDon, String ma, String ma1) throws Exception {
        try {
            String sql = "Declare @Idkh UNIQUEIDENTIFIER \n"
                    ...
                    + "update HoaDon set IdKH = @Idkh , IdNV = @Idnv, NgayThanhToan = ?,NgayDat = ?,NgayShip = ?,NgayNhan = ?,TinhTrang= ?,TongTien = ? where Ma = ?";
            Connection cn = DBConnection.getConnection();
            PreparedStatement pstm = cn.prepareStatement(sql);
            pstm.setObject(1, ma);
            ...
            pstm.setObject(7, hoaDon.getTinhTrang());
            pstm.setObject(8, hoaDon.getTongTien());
            pstm.setObject(9, hoaDon.getMaHD());
